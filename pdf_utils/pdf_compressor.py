...
def main():
    ...
    parser.add_argument('-indir', help='Path of the input directory')
    parser.add_argument('-outdir', help='Path of the output directory ')
    ...
    # In case no compression level is specified, default is 1 '/ high resolution'
    if not args.compress:
        args.compress = 1

    pdf_files = os.listdir(args.indir)

    for pdf_file in pdf_files:
        if not pdf_file.endswith(".pdf") or "merged" not in pdf_file:
            continue

        in_file = os.path.join(args.indir, pdf_file)
        out_file = os.path.join(args.outdir, pdf_file)

        if not os.path.exists(out_file):
            print("Compressing %s " % in_file)
            compress(in_file, out_file, power=args.compress)
            print("Finished compressing %s " % out_file)
        else:
            print("Skipping file %s " % in_file)
